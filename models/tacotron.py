...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None, reference_mel=None, reference_embeddings=None):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      embedding_table = tf.get_variable(
        ...
        initializer=tf.truncated_normal_initializer(stddev=0.5, seed=21))
      ...
      #隨機權重
        ...
        print("Use customize weight embeddings.")
        style_embeddings = np.ndarray([1, 1, 256])
        weights = np.loadtxt(reference_embeddings, delimiter=r' ',dtype=np.float32)
        weights = weights.reshape([4, 10])
        style_tokens_ = np.loadtxt('embeddings_database/style_tokens.txt', delimiter=r' ',dtype=np.float32)
        style_tokens_ = style_tokens_.reshape([10, 64])
        content = np.matmul(weights, style_tokens_)
        style_embeddings = content.reshape([1, 1, 256])
